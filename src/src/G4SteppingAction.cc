...
void G4SteppingAction::UserSteppingAction (const G4Step* aStep) {
    ...
    if (parti == "mu+" || parti == "millicharged")
    {
        ...
        analysisManager->FillNtupleDColumn(1,5,EDep/MeV);
        analysisManager->FillNtupleDColumn(1,6,acos(CosAngle)*180/3.1415);
        analysisManager->FillNtupleDColumn(1,7,stepLength/cm);
        analysisManager->FillNtupleDColumn(1,8,1.0);
        analysisManager->FillNtupleDColumn(1,9,1.0);
